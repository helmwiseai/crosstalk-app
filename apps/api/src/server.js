...
import { GeminiGenerator } from './providers/geminiGenerator.js';
...
const provider = (process.env.GENERATOR_PROVIDER || 'gemini').toLowerCase();
...
const turnPipeline = new TurnPipeline({
  ...
  fallbackGenerator: stubGenerator
});
...
app.get('/health', (_req, res) => {
  res.json({ ok: true, service: 'crosstalk-api', generator: provider });
});
...
app.post('/sessions/:sessionId/turn', async (req, res) => {
  ...
  const turn = await turnPipeline.run({ sessionId, userInput, inputMode: inputMode || 'text' });
