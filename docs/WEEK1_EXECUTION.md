# Week 1 Execution (PM-Coordinated)

## Objective
Stand up the MVP technical spine for Crosstalk: session lifecycle + enforceable constraints path + shared contracts.

## Ticket Set
- W1-001: Monorepo bootstrap + contracts package
- W1-002: API scaffold (`/sessions/start`, `/sessions/end`)
- W1-003: In-memory session store and target-word seed logic
- W1-004: Web shell with session state flow placeholders
- W1-005: PM docs + decision log + open questions
- W1-006: `/sessions/:id/turn` contract + endpoint stub
- W1-007: Level-0 constraint policy module scaffold

## Done This Cycle
- W1-001 ✅
- W1-002 ✅
- W1-003 ✅
- W1-004 ✅
- W1-005 ✅
- W1-006 ✅
- W1-007 ✅

## Next Immediate
- W1-008: DB schema draft (summary/exposure-first persistence)
- W1-009: Provider adapter interfaces (Gemini-first) ✅ (`GeminiGenerator` + stub fallback)
- W1-010: Local test script for full session loop ✅ (`npm run test:e2e`)
- W1-011: Replace in-memory repository with Postgres adapter
